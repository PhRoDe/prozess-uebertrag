...
| Supabase | Projekt `prozess-uebertrag` (Frankfurt, ref `msqpaiyptgrchomgdpxa`), Migrationen **manuell**. Tabellen: `jobs` (App, mit `created_by`/`company_id`) + `companies` + `industry_categories` + `line_items`/`line_item_groups` (Audit) + `pdf_extractions` (Cache) + `company_year_metrics`/`metric_definitions` (Benchmarking) + `keepalive` (Heartbeat) |
...
.venv/bin/pytest                                  # 252 Tests müssen grün sein
...
```bash
.venv/bin/pytest                      # 252 Tests (Stand 2026-06-14)
